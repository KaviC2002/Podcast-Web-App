...
def login():
    ...
    if form.validate_on_submit():
        try:
            ...
            print(f"User logged in: {session['user_name']}, ID: {session['user_id']}")
...
class PasswordValid:
    def __init__(self, message=None):
        if not message:
            message = u'Your password must be at least 8 characters, and contain a lower case letter and a digit'
